...
| Plotting/figures | Loss curves, dry-area trace, baseline comparison, benchmark-workflow, and split-diagram generation scripts; no retired-pipeline sensitivity figure is used for a current claim |
...
| Figure | Source |
|---|---|
| Ground-truth vs. prediction field snapshots (multiple horizons) | Not included in this manuscript. Tutorial tensors and compatible checkpoints are now retained in the audited local bundles; cross-condition arrays/checkpoints remain unavailable |
| Dry-area-fraction-over-time trace | Phase 4 rollout evaluation output |
| Conservation-error comparison (interface RMSE versus mass MAE, with bootstrap error bars) | Generated by `scripts/generate_submission_figures.py` from the canonical tutorial ledger |
| Loss curves (training/validation, all models/seeds) | Generated at each training phase |

Export each at minimum 300 DPI (line-art figures such as the conservation-error comparison should be vector PDF or EPS where the journal accepts it, since these need to scale cleanly in print).
...
- The frozen $\lambda_{\mathrm{div}}=.30$ CUDA confirmation table; historical sensitivity records must remain explicitly labelled as retired-pipeline selection provenance.
