...
4. **A cross-condition replication check**, in which the descriptive architecture ranking observed on the tutorial split (where T-FNO showed a non-significant edge on interface fidelity but was significantly worse on mass conservation) is re-tested under an independent protocol on two held-out conditions from the same source dataset — and reverses. On the held-out conditions, U-Net descriptively dominates across all primary metrics. The protocol also differs in spatial resolution, so this provides evidence that initial descriptive rankings in this domain can be highly sensitive to the test condition, reinforcing the need for cross-condition validation.
5. A qualitative demonstration of a mechanistically motivated conservation intervention (a spectral velocity-divergence penalty). While initial pilot sweeps show it strongly suppresses divergence and improves mass-conservation tracking descriptively, a fully powered multi-seed statistical confirmation of non-inferiority was precluded by compute-environment deprecation, leaving its formal evaluation for future work.

Consistent with contribution 4, our central finding highlights the risk of overfitting to single-condition evaluations. While T-FNO showed a promising descriptive edge on interface fidelity on the tutorial split, this did not achieve statistical significance and completely reversed on the independent cross-condition protocol, where U-Net was descriptively favored across the board. Reporting a ranking or hypothesized trade-off from a single trajectory—however statistically rigorous the seed-level inference—is therefore not sufficient evidence of a generalizable architectural property. This is a methodological finding of independent value to the broader neural-operator-for-science community, beyond its specific application to boiling.
...
where $\mathcal{K}_{\text{Tucker}}$ is the truncated Tucker-factorized spectral convolution, $\mathcal{C}_{3\times3}$ is a learned 3×3 local convolution, $W$ is the pointwise residual map, and $\phi$ is the layer nonlinearity. This architecture was initially motivated by a hypothesized tutorial-split Phase 1 Pareto trade-off that was subsequently rejected at scale (Section 4.1) — a predeclared statistical decision gate (Section 3.6) had separately and correctly rejected an earlier, unrelated intervention aimed at a non-replicating single-seed rollout artifact; the two are distinct and should not be conflated.
...
Across 11 paired seeds, T-FNO proved significantly worse than U-Net on mass-conservation MAE (+0.04917, unadjusted p=.000976562, Holm p=.0214844). It showed no statistically significant advantage on interface-temperature RMSE (−0.18853, unadjusted p=.234375, Holm p=.46875) or interface-temperature-jump MAE (−0.09277, unadjusted p=.196289, Holm p=.588867), while no overall GWRMSE winner was established (T-FNO − U-Net: +0.00460, 95% paired bootstrap CI [−0.05759, +0.05894], sign-flip p=.890625, Holm p=.890625). The fixed-split trade-off initially hypothesized is thus NOT statistically supported at this resolution. See Figure 1.
...
### 4.3 A local branch alone does not recover conservation; a targeted divergence penalty shows qualitative promise
...
An expanded sensitivity sweep of the divergence penalty (3 seeds, $\lambda_{\text{div}} \in \{.01, .03, .10, .20, .30\}$) found validation divergence decreasing monotonically through the full tested range, with 0.30 — the upper tested boundary — selected as the eligible candidate with lowest divergence. Figure 4 shows the retained sweep: the selected value lies at the tested upper boundary. **This selection identifies a value that works within the tested range; it does not identify an interior optimum, and behavior beyond 0.30 remains untested** (see Section 6, future work). While these initial pilot runs strongly suggest the divergence penalty qualitatively improves mass conservation, a fully powered 11-seed statistical confirmation was precluded by the deprecation of the required evaluation environment. We therefore report this as a promising qualitative finding requiring future statistical validation.
...
For T-FNO minus U-Net: GWRMSE +0.09448 (95% CI [+0.04363, +0.14534]), interface-temperature RMSE +0.61497 ([+0.31906, +0.90941]), jump MAE +0.18985 ([+0.18080, +0.19935]), mass MAE +0.01429 ([+0.00915, +0.02085]). Every unadjusted exact two-sided p-value is .0625 (the minimum attainable at n=5) and every Holm-adjusted p-value is 1.0. **The directions are consistent across all seeds and both held-out trajectories, and they consistently favor U-Net. This represents a descriptive reversal on interface fidelity (where T-FNO held a non-significant edge on the tutorial split),** but the five-seed design cannot establish corrected statistical significance. We report this as a clear descriptive reversal that current statistical power cannot confirm, not as an established new ranking. The per-trajectory breakdown (Twall 98 and Twall 110 separately) shows the same ordering within each condition, indicating the reversal is not driven by one anomalous trajectory, though two conditions remain too few to characterize population-level physical-regime variation.

A cross-condition comparison of the divergence-hybrid model was also attempted. However, as with the tutorial-split intervention study, this evaluation is excluded from numerical claims because it relied on the deprecated MPS pipeline. Future fully powered studies on a validated evaluation environment are required to confirm whether the qualitative conservation improvements observed in pilots transfer across physical conditions.
...
The tutorial-split result (Section 4.1) established U-Net's mass-conservation advantage under a rigorous multi-seed protocol, but T-FNO's hypothesized interface-fidelity advantage failed to reach statistical significance. It is also, on its own, an incomplete basis for an architecture recommendation: T-FNO's descriptive edge on interface fidelity completely reversed under this paper's independent cross-condition protocol (Section 4.4), which also differs in spatial resolution. Thus the result does not isolate wall temperature or any other individual physical factor as the cause of the change. It does show that **a statistically well-powered single-trajectory comparison, however rigorously analyzed at the seed level, does not by itself establish a generalizable architectural property** in this domain. Seed-level rigor and condition-level rigor are separate axes of evidence.
...
For practitioners, the safest actionable guidance from this study is: do not select an architecture for phase-resolved boiling forecasting based on a single trajectory's results, regardless of how many seeds or how rigorous the statistical test at that trajectory. Where an explicit conservation requirement exists, a targeted physical penalty (Section 4.3–4.4) currently appears to be the more robust lever than architectural locality alone, though this too awaits confirmation at higher cross-condition statistical power. Where interface fidelity is paramount, the apparent descriptive edge of an architecture like T-FNO on a single condition should not be assumed to hold statistically or to generalize to a different operating point without direct testing, exactly as this paper's own result demonstrates (showing a complete descriptive reversal).
