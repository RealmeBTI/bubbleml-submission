...
\emph{Full manuscript draft. Bibliographic details (exact author lists,
years, venues, DOIs) must be verified against original publisher records
before submission --- citations below reflect research conducted during
this project\textquotesingle s development, not a formatted reference
manager output. All local file paths referenced are internal
reproducibility pointers for the authors and should be replaced with a
public repository link before submission.}
...
Zuber\textquotesingle s hydrodynamic instability model. Subsequent
decades of work have produced a large family of semi-empirical
correlations tuned to specific fluids, geometries, and flow regimes.
These remain the workhorse of engineering practice due to their
computational simplicity, but they are aggregate, steady-state
predictors that do not resolve transient local bubble dynamics on a
specific surface, and they offer no natural mechanism for real-time,
sensor-driven early warning.
...
coupled to the Navier-Stokes and energy equations. The BubbleML dataset,
...
The Fourier Neural Operator performs global convolution via pointwise
...
Convolutional U-Net architectures instead rely on local receptive fields
...
a low-rank Tucker decomposition; axis-factorized FNO (F-FNO) factorizes
...
or discontinuity issue. U-shaped FNO (UNO) grafts U-Net-style
multi-scale structure onto Fourier blocks; more recent local-global
hybrid architectures explicitly combine spectral and convolutional
branches within each operator layer, motivated by the same diagnosis
this paper\textquotesingle s local-global hybrid is built around.
Transformer-based operators, including a recent boiling-specific
transformer forecaster (Bubbleformer) benchmarked on the extended
...
this paper performs; nor has the literature we are aware of examined
conservation-preserving operator variants (e.g., hard-constrained or
projection-based conservation layers) specifically for phase-resolved
boiling, which we note as an open direction distinct from the soft
penalty approach used here.

\subsection*{2.3 Experimental boiling
diagnostics}\label{23-experimental-boiling-diagnostics}

Independent of the simulation-data operator-learning literature, work
using high-speed infrared or optical measurements of real boiling
surfaces has applied convolutional networks to dry-area detection and
margin-to-CHF estimation from experimental, not simulated, data. This
experimental line of work and the simulation-based operator-learning
line of work reviewed above remain largely disjoint; no result in the
present paper establishes transfer to experimental measurements or
deployment-grade CHF warning.
...
regimes, and evaluation conventions, a fragmentation documented in a
recent survey of open multimodal boiling and phase-change datasets. This
work\textquotesingle s explicit, checksum-verified, fully reproducible
data and evaluation pipeline is designed to partially mitigate this
within its own scope. Combined with the cross-condition replication
check described above, we position this work primarily as a benchmark
and evaluation-methodology contribution --- a genuinely rigorous
protocol for making and testing architecture claims on chaotic,
multiphysics surrogate tasks --- rather than as a claim of a novel
architecture. The individual architectural components used here
(Tucker/factorized FNO, local-global hybridization, spectral divergence
penalties, bounded output heads) are each drawn from or closely related
to existing techniques; the contribution is their careful, statistically
disciplined integration and, specifically, the demonstration that even a
well-powered single-condition comparison among them is not sufficient
evidence of a generalizable ranking.
...
(T-FNO, rank 0.1), axis-factorized FNO (F-FNO), and a
...
{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}P{0.18\textwidth}P{0.47\textwidth}P{0.27\textwidth}@{}}
\toprule\noalign{}
...
All data sources are checksummed (Section 3.1); all model
configurations, seeds, training histories, and checkpoints are logged
per experiment; all statistical comparisons use a single, unchanged,
audited implementation (Section 3.6) across every reported result. A
full artifact manifest --- including per-seed JSON results, benchmark
CSVs, training curves, and the acquisition and preprocessing scripts for
both the tutorial and legacy-archive datasets --- accompanies this
manuscript and should be released as a public repository prior to
submission, replacing the internal file paths referenced during
development.
