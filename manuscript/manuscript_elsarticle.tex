...
\fntext[orcid]{ORCID: 0009-0009-6695-365X}
...
\emph{S. B. Mahafuj Bondhon, Department of Mechanical Engineering,
Bangladesh University of Engineering and Technology (BUET), Ramna,
Dhaka-1000, Bangladesh. Corresponding author:
\href{mailto:2210062@me.buet.ac.bd}{\nolinkurl{2210062@me.buet.ac.bd}}.
ORCID: 0009-0009-6695-365X. The bibliography entries in the generated
manuscript were checked against the primary records documented in
\texttt{BIBLIOGRAPHY\_VERIFICATION.md}. The prior public repository
release is
\url{https://github.com/RealmeBTI/bubbleml-submission/releases/tag/v1.0.4};
its matching Zenodo version archive is
\url{https://doi.org/10.5281/zenodo.21858198}. The archive/version scope
and the retained-artifact limitations are recorded in
\texttt{PRESUBMISSION\_FINAL\_CHECK.md}.}
...
learned directly from CFD trajectories, provided they satisfy local conservation and stability requirements over long autoregressive horizons. This poses a fundamental methodological challenge for the field: even with rigorous multi-seed testing, are well-powered, single-condition benchmarks sufficient evidence for general architecture claims in neural PDE surrogates, or do they risk overfitting to the specific physical regime being evaluated? The Fourier Neural Operator
...
\item
  ...
  effect.
\item
  \textbf{A cross-condition replication check}, in which the descriptive
  architecture ranking observed on the tutorial split (where T-FNO showed a
  non-significant edge on interface fidelity but was significantly worse
  on mass conservation) is re-tested under an independent protocol on two
  held-out conditions from the same source dataset --- and reverses. On the
  held-out conditions, U-Net descriptively dominates across all primary metrics.
  The protocol also differs in spatial resolution, so this provides evidence
  that initial descriptive rankings in this domain can be highly sensitive
  to the test condition, reinforcing the need for cross-condition validation.
\item
  A qualitative demonstration of a mechanistically motivated conservation intervention (a spectral velocity-divergence penalty). While initial pilot sweeps show it strongly suppresses divergence and improves mass-conservation tracking descriptively, a fully powered multi-seed statistical confirmation of non-inferiority was precluded by compute-environment deprecation, leaving its formal evaluation for future work.
\end{enumerate}

Consistent with contribution 4, our central finding highlights the risk
of overfitting to single-condition evaluations. While T-FNO showed a
promising descriptive edge on interface fidelity on the tutorial split,
this did not achieve statistical significance and completely reversed on
the independent cross-condition protocol, where U-Net was descriptively
favored across the board. Reporting a ranking or hypothesized trade-off
from a single trajectory---however statistically rigorous the seed-level
inference---is therefore not sufficient evidence of a generalizable
architectural property. This is a methodological finding of independent
value to the broader neural-operator-for-science community, beyond its
specific application to boiling.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{../submission/figures/fig1_pareto_front.pdf}
\caption{The retained tutorial-split results place T-FNO and
U-Net on opposing sides of the interface-fidelity/mass-conservation
trade-off; the local-global variants are shown for the intervention
context. This figure summarizes the fixed-split result only and does not
establish a cross-condition ranking.}
\label{fig:pareto-tradeoff}
\end{figure}
...
train/validation/test. The distinct tutorial and cross-condition
protocols are visualized in Figure~\ref{fig:split-design}. A separate one-epoch,
batch-size-one feasibility check was also run at native 384\ensuremath{\times}384
resolution to test direct execution and memory feasibility only (not
convergence or ranking).
...
models train at full 48\ensuremath{\times}48 (tutorial) or 96\ensuremath{\times}96 (cross-condition)
resolution with 24\ensuremath{\times}24 real-FFT modes --- the maximum representable given
...
layer nonlinearity. This architecture was initially motivated by a hypothesized
tutorial-split Phase 1 Pareto trade-off that was subsequently rejected at scale
(Section 4.1) --- a predeclared statistical decision gate (Section 3.6)
had separately and correctly rejected an earlier, unrelated intervention
aimed at a non-replicating single-seed rollout artifact; the two are
...
hybrid (Section 3.4) and divergence penalty (Section 3.5) were motivated
by early observations of a potential tutorial-split Pareto trade-off
and are not affected by that earlier gate's rejection.
...
targeted divergence penalty shows qualitative promise}
...
An expanded sensitivity sweep of the divergence penalty (3 seeds,
\(\lambda_{\text{div}} \in \{.01, .03, .10, .20, .30\}\)) found validation divergence
...
lowest divergence. Figure~\ref{fig:lambda-sensitivity} shows the retained sweep: the selected value
lies at the tested upper boundary. \textbf{This selection identifies a
value that works within the tested range; it does not identify an
interior optimum, and behavior beyond 0.30 remains untested} (see
Section 6, future work). While these initial pilot runs strongly suggest the divergence penalty qualitatively improves mass conservation, a fully powered 11-seed statistical confirmation was precluded by the deprecation of the required evaluation environment. We therefore report this as a promising qualitative finding requiring future statistical validation.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{../submission/figures/fig3_lambda_sensitivity.pdf}
\caption{The expanded sensitivity sweep reports validation
MSE, spectral divergence MAE, and interface-temperature RMSE for each
tested divergence-penalty weight. The red line marks the selected
upper-bound value, \(\lambda_{\text{div}}=.30\); the sweep does not
establish behavior beyond that range.}
...
and both held-out trajectories, and they consistently favor U-Net. This
represents a descriptive reversal on interface fidelity (where T-FNO
held a non-significant edge on the tutorial split),} but the five-seed design cannot establish corrected
...
ranking. The per-trajectory breakdown (Twall 98 and Twall 110
...
The retained tutorial validation histories are shown in Figure~\ref{fig:training-histories}. They
document the available tutorial training runs but do not establish
cross-condition convergence, because the complete cross-condition
per-seed histories are not retained locally.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{../submission/figures/fig4_loss_curves.pdf}
\caption{Per-seed validation histories for the retained Phase
1 tutorial-split runs. These curves provide training-trace context for
the stored tutorial results only; they are not a substitute for
unavailable cross-condition training histories.}
\label{fig:training-histories}
\end{figure}
...
The tutorial-split result (Section 4.1) established U-Net's
mass-conservation advantage under a rigorous multi-seed protocol, but
T-FNO's hypothesized interface-fidelity advantage failed to reach
statistical significance. It is also, on its own, an incomplete
basis for an architecture recommendation: T-FNO's descriptive edge
on interface fidelity completely reversed under this paper\textquotesingle s
independent cross-condition protocol (Section 4.4), which also differs in
spatial resolution. Thus the result
does not isolate wall temperature or any other individual physical
factor as the cause of the change. It does show that \textbf{a
statistically well-powered single-trajectory comparison, however
rigorously analyzed at the seed level, does not by itself establish a
generalizable architectural property} in this domain. Seed-level rigor
and condition-level rigor are separate axes of evidence.
...
Unlike the architecture-level interface-fidelity result (which reversed
across conditions), the divergence
...
Reproducibility & Retained tutorial and intervention data/result
artifacts are checksummed; complete cross-condition per-seed histories
and checkpoints are unavailable & Stored-result self-test is
reproducible; full raw-data-to-checkpoint reproduction remains
unavailable \\
Exploratory resolution comparison & The 48\ensuremath{\times}48 and
96\ensuremath{\times}96 stored configurations differ in temporal frame
range, hardware, source revision, and checkpoint availability in
addition to spatial resolution; no factorial resolution-only control
has been executed & Reported as descriptive only; a fully controlled
resolution experiment is identified as future work (Section 6) \\
\end{longtable}
}
...
paramount, the apparent descriptive edge of an architecture like T-FNO
on a single condition should not be assumed to hold statistically or
to generalize to a different
operating point without direct testing, exactly as this paper\textquotesingle s
own result demonstrates (showing a complete descriptive reversal).
...
\item
  A factorial resolution experiment --- training the same model
  configurations on the same trajectories with matched temporal windows,
  matched hardware, matched checkpointing, and a predeclared
  architecture\ensuremath{\times}resolution statistical interaction test --- to determine
  whether the directional differences observed in Section 4.7 are
  attributable to spatial resolution specifically, or to the other
  pipeline factors that currently co-vary with it.
\end{itemize}
...
acquisition/preprocessing scripts for both data formats. The prior
public \texttt{v1.0.4} repository release is archived at Zenodo, DOI
\url{https://doi.org/10.5281/zenodo.21858198}. That archive is a
version-specific snapshot and does not remove the stated raw-data,
checkpoint, cross-condition-provenance, or field-snapshot limitations.
