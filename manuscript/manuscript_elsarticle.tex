...
\item
  ...
  U-Net favors mass conservation) on one trajectory split is re-tested
  under an independent protocol on two held-out conditions from the same
  source dataset --- and does not reproduce. The protocol also differs
  in spatial resolution, so this is evidence against generalizing the
  tutorial ranking, not evidence that wall temperature alone caused the
  ranking change.
\item
...
winning architecture. The tutorial ranking did not replicate under an
independent cross-condition protocol that also differed in spatial
resolution. Reporting a ranking from a single trajectory---however
statistically rigorous the seed-level inference---is therefore not
sufficient evidence of a generalizable architectural property. This is a
methodological finding of independent value to the broader
neural-operator-for-science community, beyond its specific application
to boiling.
...
\subsection*{3.8 Exploratory same-trajectory resolution
comparison}\label{38-exploratory-same-trajectory-resolution-comparison}

As a post-hoc exploratory observation, T-FNO and U-Net were also trained
and evaluated on the same three tutorial trajectories (Twall-103/106/100)
at 96\ensuremath{\times}96 resolution using identical random seeds (n=11 paired seeds:
7, 17, 42, 100, 314, 1234, 2025, 2718, 4242, 7777, 9999) and the same
model configurations and evaluation metrics as the 48\ensuremath{\times}48 tutorial
split. This configuration is archived on the
\texttt{experiment/resolution-control-n11} branch.

This comparison is \textbf{not} a controlled resolution experiment.
Multiple factors differ between the 48\ensuremath{\times}48 and 96\ensuremath{\times}96 configurations
beyond spatial resolution: the temporal frame ranges differ (169 frames,
timesteps 30--198, at 48\ensuremath{\times}48 versus 166 frames, timesteps 30--195, at
96\ensuremath{\times}96), the hardware environments differ (Apple M2/MPS at 48\ensuremath{\times}48
versus NVIDIA Tesla T4/CUDA at 96\ensuremath{\times}96), source code revisions differ,
and 48\ensuremath{\times}48 model checkpoints are retained whereas the 96\ensuremath{\times}96
checkpoints are not available in the authorized artifact set. The result
should be interpreted as a descriptive observation across two stored
pipeline configurations that differ in resolution among other factors,
not as evidence that spatial resolution alone caused any observed
difference. A full control-matrix documenting all factor differences is
retained in \texttt{audit\_n11/control\_matrix.md}.
...
The fixed-split trade-off is shown in Figure~\ref{fig:pareto-tradeoff}.
...
Section 6, future work). A fresh 11-seed confirmation at
\(\lambda_{\text{div}}=.30\) passed the same predeclared criterion with
a substantially larger margin: mean mass MAE 0.09373 vs.
U-Net\textquotesingle s 0.16586 (paired difference -0.07212, 95\% CI
{[}-0.07940, -0.06535{]}, p=.000488), with no established interface
regression relative to the zero-penalty hybrid. We adopt
\(\lambda_{\text{div}}=.30\) as the primary divergence-hybrid
...
\subsection*{4.7 Exploratory cross-resolution comparison on tutorial
trajectories (not a controlled resolution
experiment)}\label{47-exploratory-cross-resolution-comparison}

For the same tutorial trajectories (Twall-103/106/100) trained at
96\ensuremath{\times}96 resolution (Section 3.8), the stored n=11 paired T-FNO minus
U-Net differences are:

\begin{itemize}
\tightlist
\item GWRMSE: $+0.10868$ (Holm p\,=\,0.00391; favors U-Net)
\item Interface-temperature RMSE: $+0.46219$ (Holm p\,=\,0.00977; favors U-Net)
\item Interface-temperature-jump MAE: $+0.21433$ (Holm p\,=\,0.00391; favors U-Net)
\item Mass-conservation MAE: $+0.04906$ (Holm p\,=\,0.00391; favors U-Net)
\end{itemize}
...
All four metrics favor U-Net in the 96\ensuremath{\times}96 stored configuration, with
Holm-adjusted p-values well below 0.05 --- a marked contrast to the
48\ensuremath{\times}48 tutorial split, where T-FNO was significantly better on
interface-temperature metrics (Section 4.1). For mass conservation, both
resolutions return a positive difference (T-FNO worse than U-Net), so
the direction holds across configurations for that metric; only the
magnitude differs ($+0.04494$ at 48\ensuremath{\times}48 versus $+0.04906$ at
96\ensuremath{\times}96).

These differences are stored-pipeline observations that reflect the
combined effect of resolution and the other pipeline factors documented
in Section 3.8 and \texttt{audit\_n11/control\_matrix.md}. They cannot
be attributed to spatial resolution as an isolated cause. We report them
explicitly as exploratory because the statistical power (n=11) is
sufficient to detect the signed direction reliably, but the absence of
experimental controls prevents any causal interpretation. The correct
scientific reading is: \textit{the relative T-FNO/U-Net paired
differences observed in the 96\ensuremath{\times}96 stored pipeline configuration are
consistently opposite in sign to the 48\ensuremath{\times}48 interface-fidelity
results, and this observation motivates a future dedicated study that
would need to control the confounding pipeline factors before attribution
is possible.}
...
The tutorial-split result (Section 4.1) is statistically confirmed under
a rigorous multi-seed protocol. It is also, on its own, an incomplete
basis for an architecture recommendation: the ranking did not replicate
under this paper\textquotesingle s independent cross-condition protocol
(Section 4.4), which also differs in spatial resolution. Thus the result
...
The exploratory same-trajectory 96\ensuremath{\times}96 comparison (Section 4.7)
produces directionally consistent results with the multi-trajectory
cross-condition test (Section 4.4): both stored 96\ensuremath{\times}96 configurations
show positive T-FNO $-$ U-Net paired differences on interface-temperature
metrics, opposite to the tutorial-split 48\ensuremath{\times}48 result. We treat this
consistency as hypothesis-generating rather than confirmatory, because
the 96\ensuremath{\times}96 stored pipeline configurations differ from the 48\ensuremath{\times}48
configuration in multiple factors beyond resolution (Section 3.8); no
factorial design isolating resolution has been executed. The observation
nevertheless strengthens the motivation for a future controlled study
(Section 6).
...
The exploratory same-trajectory 96\ensuremath{\times}96 comparison (Section 4.7) is
archived on the \texttt{experiment/resolution-control-n11} branch at
commit \texttt{eb7884e}. This archive includes per-seed
\texttt{config.yaml}, \texttt{results.json}, and training curve
files for all 11 T-FNO and 11 U-Net seeds, the merged
\texttt{benchmark\_results.json} and \texttt{benchmark\_summary.csv},
and the \texttt{resolution\_control\_analysis.json} containing the
computed paired differences and Holm-adjusted p-values. The 96\ensuremath{\times}96
model checkpoints are not committed (Kaggle-resident only). The control
matrix documenting all pipeline differences is retained in
\texttt{audit\_n11/control\_matrix.md}. These stored artifacts are
reproducible at the analysis and summary level from the committed JSON
files; full end-to-end reproducibility from raw data requires
externally hosted data, checkpoints, and the Kaggle GPU environment.
